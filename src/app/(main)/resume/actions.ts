...
export async function saveGitLinkAction(
  ...
  // Git 링크는 README 없거나 fetch 실패해도 저장은 진행 — 사용자가 URL을 명시했으니까.
  // 다만 parsedText가 비었으면 normalize는 처음부터 'failed'로 표기한다.
  const initialStatus: NormalizeStatus = parsedText ? "pending" : "failed";
  const doc = await createDocument({
    user_id: user.id,
    type: "git",
    file_url: trimmed,
    file_name: trimmed,
    parsed_text: parsedText,
    normalize_status: initialStatus,
  });

  if (parsedText) {
    after(async () => {
      try {
        const normalized = await runNormalize(parsedText, "git", BACKGROUND_NORMALIZE_TIMEOUT_MS);
        if (normalized) {
          await updateNormalizedText(doc.id, normalized);
        } else {
          await markNormalizeFailed(doc.id);
